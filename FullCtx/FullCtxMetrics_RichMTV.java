...
/**
 * Diagnostic only: FullCtx (r^C) MAE/RMSE/AUC/coverage on RichMTV_clean.csv,
 * measured on one held-out 80/20 split (seed=1), consistent with
 * FullCtxCoverage_RichMTV.java's split. Does not modify any pipeline file.
 */
public class FullCtxMetrics_RichMTV {
    static int N_DIMS = 8;
    ...
    public static void main(String[] args) throws Exception {
        ...
        int userMeanFallback = 0, globalMeanFallback = 0;
        ...
        for (int[] tst : test) {
            ...
            if (cands == null || cands.isEmpty()) noCoraters++; else coratersButNoMatch++;
        ...
        // AUC: per-user, positives = test items rated >= RATING_THRESHOLD,
        // negatives = NEG_SAMPLE sampled items not rated by the user (train+test).
        ...
        System.out.printf("  Fallback (co-raters exist, no match): %.2f%%%n", 100.0*coratersButNoMatch/nTest);
        System.out.printf("  Fallback (no co-raters at all): %.2f%%%n", 100.0*noCoraters/nTest);
