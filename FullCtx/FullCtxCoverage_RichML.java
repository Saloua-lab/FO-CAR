...
/**
 * Diagnostic only: measures FullCtx (r^C) coverage on RichMovieLens_clean.csv.
 * Does not touch any prediction/scoring logic in the actual pipeline files.
 *
 * "Coverage" = fraction of test rows for which at least one training co-rater
 * (same item, different user) matches the target's context on ALL N_DIMS
 * dimensions exactly (the condition predictForShapley(..., allDims=true, ...)
 * requires to produce a real average instead of falling back to uMean).
 */
public class FullCtxCoverage_RichML {
    static int N_DIMS = 9;

    public static void main(String[] args) throws Exception {
        ArrayList<int[]> all = loadData("RichMovieLens_clean.csv");
        ...
        int nTest = test.size();
        int noCoraters = 0;      // item has zero training co-raters at all
        int coratersButNoMatch = 0; // has co-raters, but none match all dims
        int fullMatch = 0;       // at least one co-rater matches all dims exactly
