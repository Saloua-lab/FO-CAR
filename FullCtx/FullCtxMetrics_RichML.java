...
/** Diagnostic only: AUC for FullCtx (r^C) on RichMovieLens_clean.csv (single 80/20 split, seed=1). */
public class FullCtxMetrics_RichML {
    static int N_DIMS = 9;
    static int NEG_SAMPLE = 1000;
    static int RATING_THRESHOLD = 8; // x2 scale (=4 stars)
    ...
    public static void main(String[] args) throws Exception {
        ...
        float gMean = gCnt > 0 ? gSum / gCnt : 6f;
        ...
        float mae = (sumAbs / nTest) / 2f;
        float rmse = ((float) Math.sqrt(sumSq / nTest)) / 2f;
