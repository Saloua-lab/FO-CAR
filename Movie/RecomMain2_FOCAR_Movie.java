 ...
 *
 * Dataset: DePaulMovie (3 context dims: Time, Location, Companion; ratings 1â€“5)
 * CSV columns: userId,itemId,rating,time,location,companion
 */
public class RecomMain2_FOCAR_Movie {

    static String DATA_CSV = "D:\\datasets\\DePaulMovie_clean.csv";
    static String OUT_FILE = "D:\\datasets\\Movie_FOCARS_CV.txt";
    ...
    static double EPSILON   = 0.1;  // 1/N_DIMS=0.333 for 3 dims

    private static double[] lastMetrics;

    //  MAIN
    public static void main(String[] args) throws Exception {
        System.out.println("=== FO-CAR DePaulMovie (paper-aligned pipeline) ===\n");
        ...
        StringBuilder sb = new StringBuilder("=== FO-CAR DePaulMovie RESULTS ===\n");
        for (int m = 0; m < 10; m++) {
    ...
    //  Data loading â€“ handles string item IDs and categorical context dims
    @SuppressWarnings("unchecked")
    private static ArrayList<int[]> loadData(String path) throws Exception {
        ArrayList<int[]> all = new ArrayList<int[]>();
        Map<String, Integer> itemEnc = new HashMap<String, Integer>();
        int[] nextItem = {1};
        Map<String, Integer>[] ctxEnc = new HashMap[N_DIMS];
        int[] nextCtx = new int[N_DIMS];
        for (int d = 0; d < N_DIMS; d++) { ctxEnc[d] = new HashMap<String, Integer>(); nextCtx[d] = 1; }
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (uid < 0) continue;
            // Item ID may be a string (movie title)
            String itemStr = c[1].trim();
            int iid = parseIntSafe(itemStr);
            if (iid < 0) {
                Integer code = itemEnc.get(itemStr);
                if (code == null) { code = nextItem[0]++; itemEnc.put(itemStr, code); }
                iid = code;
            }
            int rat = parseIntSafe(c[2].trim());
            if (rat < 0 || rat > 5) continue;
            ...
            for (int d = 0; d < N_DIMS; d++) {
                ...
                if (val.isEmpty() || val.equalsIgnoreCase("NA") || val.equalsIgnoreCase("null")
                    ...
                    if (v >= 0) {
                        ...
                        Integer code = ctxEnc[d].get(val);
                        if (code == null) { code = nextCtx[d]++; ctxEnc[d].put(val, code); }
                        row[3 + d] = code;
                    }
                }
            }
