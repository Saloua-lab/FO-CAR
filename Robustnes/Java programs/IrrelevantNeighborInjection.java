...
/**
 * FO-CAR -- Irrelevant-Neighbor Injection Experiment.
 *
 * Directly tests "CINS reduces the impact of irrelevant neighbors" without
 * any adversarial/malicious-user framing:
 *
 *   1. Take the real Pearson neighborhood (step-2 co-raters with pearson>0
 *      against the target user -- genuine, positively-correlated raters).
 *   2. Inject synthetic "irrelevant" duplicates: clone a subset of those same
 *      real candidates (same user, same rating -- nothing fabricated) but
 *      overwrite their context vector so it matches the target situation on
 *      ZERO dimensions. Same rater, same taste correlation, just the wrong
 *      context for this particular prediction.
 *   3. Measure what fraction of the injected duplicates get choquet < EPSILON
 *      (i.e. are actually dropped by CINS's filter).
 *   4. Compare MAE/RMSE degradation (no-injection -> +50% injected) between
 *      CINS and a plain Pearson-kNN (No-CINS) arm on the SAME injected pool.
 *
 * Injection rate is relative to the real candidate pool size (0.5 = add
 * irrelevant duplicates equal to 50% of the real neighborhood).
 */
public class IrrelevantNeighborInjection {
    ...
    // Influence Ratio (IR) = sum of injected-neighbor weight / sum of total
    // neighbor weight, aggregated over all "+injection" predictions. Measures
    // down-weighting directly (unlike the elimination count, which only sees
    // the binary in/out decision) -- IR(CINS) < IR(No-CINS) would show CINS
    // assigns injected/irrelevant neighbors less relative influence even when
    // it doesn't exclude them outright.
    private static double statWtotalNoCins = 0, statWinjNoCins = 0;
    private static double statWtotalCins   = 0, statWinjCins   = 0;

    public static void main(String[] args) throws Exception {
        System.out.println("=== FO-CAR CINS -- Irrelevant-Neighbor Injection Experiment ===\n");
        ...
        // Settings: 0=noInject_noCINS, 1=noInject_CINS, 2=inject_noCINS, 3=inject_CINS
        // out per fold = [MAE0,RMSE0, MAE1,RMSE1, MAE2,RMSE2, MAE3,RMSE3]
        ...
        writeCsv(avg, deltaNoCins, deltaCins, elimRate, irNoCins, irCins, "irrelevant_neighbor_injection_results.csv");
    }
    ...
    // Returns [MAE0,RMSE0, MAE1,RMSE1, MAE2,RMSE2, MAE3,RMSE3]
    private static double[] evalFold(ArrayList<int[]> train, ArrayList<int[]> test, int seed, int fold) {
        ...
        // Observed value pool per context dimension (for building context-mismatched
        // injected neighbors), computed from train only -- same approach as the
        // spurious-attack noise injector in RichMTV_Robustness_1.java.
        ...
        // Per-user row index (train only): lets injected candidates borrow a
        // REAL alternate (rating, context) pair from the same underlying user's
        // own history, instead of a synthetically-perturbed clone.
        ...
        for (int[] tst : test) {
            ...
            long[] injStats = new long[2]; // [totalInjected, eliminated]
            double[] weightStats = new double[4]; // [Wtotal_noCINS, Winj_noCINS, Wtotal_CINS, Winj_CINS]
        ...
        System.out.printf(Locale.US, "SEED=%d FOLD=%d MAE(noCINS/CINS no-inject)=%.4f/%.4f MAE(noCINS/CINS +inject)=%.4f/%.4f injected-eliminated=%d/%d (%.1f%%) (%ds)%n",
            seed, fold, out[0], out[2], out[4], out[6], foldInjectedEliminated, foldInjectedTotal, elimPct, dt/1000);
        return out;
    }

    /**
     * Computes all 4 predictions (no-injection x {No-CINS,CINS}, +injection x
     * {No-CINS,CINS}) from a single shared candidate build. injStatsOut[0]/[1]
     * receive [totalInjected, eliminatedByChoquet]; weightStatsOut[0..3]
     * receive [Wtotal_noCINS, Winj_noCINS, Wtotal_CINS, Winj_CINS] for the
     * "+injection" predictions, for this one prediction.
     */
    private static void predictWithInjection(int uid, float uMean, int[] ctx, ArrayList<int[]> candidates,
        ...
        // -- Build injected pool. Primary mechanism: borrow a REAL alternate
        //    (rating, context) row from the same underlying user's own train
        //    history (a genuinely different situation they rated) -- this gives
        //    naturally-varying context overlap with the target (not a forced
        //    all-zero match) and an independent rating value (not a duplicate
        //    of already-counted signal), rather than a synthetic clone. Falls
        //    back to a synthetic majority-mismatch only if that user has no
        //    other row in train to draw from. --
        ...
        for (int k = 0; k < nInject; k++) {
            ...
            if (altRow != null) {
                ...
                injNb[0] = baseUid;
                injNb[1] = baseNb[1];
                injNb[2] = altRow[2]; // real rating from that OTHER situation
            ...
            fdAll.add(fd);
            // Pearson reflects the real, full rating-history correlation of this
            // user with the target user -- independent of which row we borrow.
            pearsonAll.add(pearsonList.get(baseIdx));
            nbAll.add(injNb);
        }

        // -- With injection -- (also collect Influence Ratio weight stats:
        // total weight vs. weight coming from the injected candidates only)
        double[] wStatsNoCins = new double[2]; // [Wtotal, Winj]
    ...
    /**
     * Same as weightedPred, but also reports the Influence Ratio inputs:
     * weightStatsOut[0] = total weight summed over all neighbors,
     * weightStatsOut[1] = weight summed over neighbors at index >= injectStart
     * (the injected ones). Indices [0, injectStart) are real, [injectStart, ..)
     * are injected -- matches how nbAll/pearsonAll are laid out in the caller.
     */
    ...
    /** Same as weightedPredCINS, with the Influence Ratio inputs (see weightedPredWithInfluence). */
